...
class MQTTControl:
    ...
    def notify_brightness(self, message, qos=0):
        """
        send to the MQTT brightness channel: MQTT Clients will be informed about brighness Changes

        :param message: the text to be displayed
        :param qos: MQTT parameter
        """
        paho.mqtt.publish.single(
            topic=self.conf.MQTT.brightness_path.format(prefix=self.conf.MQTT.prefix, sys_name=self.conf.sys_name),
            payload=message,
            qos=qos,
            hostname=self.conf.MQTT.Broker.host,
            port=self.conf.MQTT.Broker.port,
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        start_path = self.conf.MQTT.general_path.format(
            ...
            show_name="+",
            command="start")
        startnojson_path = self.conf.MQTT.general_path.format(
            prefix=self.conf.MQTT.prefix,
            sys_name=self.conf.sys_name,
            show_name="+",
            command="startnojson")
        ...
        client.subscribe(startnojson_path)
        client.subscribe(stop_path)
        logger.info("subscription on Broker {host} for {start_path}, {startnojson_path} and {stop_path}".format(
            host=self.conf.MQTT.Broker.host, start_path=start_path,  startnojson_path=startnojson_path, stop_path=stop_path))
